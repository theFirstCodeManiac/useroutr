...
const products = [
  {
    ...
    description:
      "One-click checkout for cards, bank transfers, and 20+ crypto assets. Non-custodial, branded to your UI, and blazing fast.",
    icon: CreditCard,
  ...
  },
  {
    ...
    description:
      "Send instant payouts to 174 countries. Bank rails or mobile money, settled via Stellar USDC.",
    icon: Globe,
    tag: "Network",
  ...
  },
  {
    ...
    description:
      "Programmatic invoices that update in real-time. HTLC-secured payments with automated reconciliation.",
    icon: FileText,
    tag: "Automation",
    span: "md:col-span-1 lg:col-span-1",
  ...
  },
  {
    title: "On/Off Ramps",
    description:
      "Cash-to-crypto via MoneyGram's global network. Zero-friction ramps for every user.",
    icon: RefreshCw,
  ...
  },
  {
    ...
    description:
      "The entire Useroutr stack under your brand. Dedicated liquidity pools and custom compliance modules.",
    icon: Palette,
    tag: "Enterprise",
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 h-full flex flex-col">
        ...
        <div className="font-mono text-[10px] text-teal/80 mb-2.5 uppercase tracking-[0.15em] font-medium">
          {p.tag}
        </div>
        <h3 className="font-display text-[20px] font-bold text-white mb-3 tracking-tight">
          {p.title}
        </h3>
        <p className="font-serif font-light text-[15px] text-body leading-relaxed mb-8 grow">
          {p.description}
        </p>
    ...
    <div className="relative min-h-screen bg-ink selection:bg-blue/30 selection:text-white">
      ...
      <nav
        className={cn(
          ...
          scrolled
            ? "bg-ink/90 backdrop-blur-3xl border-b border-rule"
      ...
      >
        <div className="flex items-center justify-between w-full max-w-[1200px] mx-auto">
          <a href="#" className="flex items-center gap-2.5 no-underline group">
            ...
            <span className="font-display text-[17px] font-bold text-lead tracking-tight">
              useroutr
            </span>
          </a>

          <ul className="hidden md:flex items-center gap-7 list-none">
            {["How it works", "Products", "Developers", "Pricing"].map(
              (item) => (
                <li key={item}>
                  <a
          ...
          <div className="flex items-center gap-3">
            <a
              href="#"
              className="hidden sm:inline-block font-display text-[13px] font-semibold text-body hover:text-lead transition-colors px-4 py-2"
            >
              Sign in
            </a>
            <a
      ...
      <section className="relative min-h-svh flex flex-col justify-center px-6 md:px-10 py-32 overflow-hidden">
        ...
        <div className="relative z-1 w-full max-w-[1100px] mx-auto">
          ...
          >
            Pay{" "}
            <span className="font-serif font-light italic text-transparent [-webkit-text-stroke:1px_rgba(255,255,255,0.5)] tracking-[-0.02em]">
              anything.
            </span>
            <br />
            Settle{" "}
            <span className="bg-linear-to-r from-blue2 to-teal bg-clip-text text-transparent">
              everywhere.
            </span>
          </motion.h1>
          ...
          >
            ...
            <strong className="text-lead font-normal">
              both sides of finance.
            </strong>
            Accept any currency from any chain. Settle globally in seconds. One
            API. Zero custody risk.
          </motion.p>
          ...
          >
            <a
              href="#waitlist"
              className="bg-blue hover:bg-blue2 text-white font-display text-[15px] font-semibold px-7 py-3.5 rounded-[10px] transition-all hover:-translate-y-0.5 hover:shadow-xl hover:shadow-blue/30"
            >
              Join the waitlist
            </a>
            <a
              href="#"
              className="border border-rule2 bg-transparent hover:bg-white/5 hover:border-white/30 text-lead font-display text-[15px] font-semibold px-7 py-3.5 rounded-[10px] transition-all"
            >
              Read the docs →
            </a>
          </motion.div>
          ...
          >
            {chains.map((chain) => (
              <div
                key={chain.name}
                className="flex items-center gap-2 bg-ink2 border border-rule hover:border-rule2 rounded-full px-3.5 py-1.5 transition-all group"
              >
                <span
                  className="w-[7px] h-[7px] rounded-full shrink-0"
                  style={{ background: chain.color }}
                />
                <span className="font-mono text-[12px] text-body group-hover:text-lead transition-colors">
                  {chain.name}
                </span>
              </div>
            ))}
          ...
          >
            {[
              ...
              <div
                key={i}
                className="flex-1 min-w-[150px] pr-10 border-r border-rule mr-10 last:border-none last:mr-0 last:pr-0 pb-6 md:pb-0"
              >
                ...
                <div className="font-mono text-[12px] text-faint tracking-[0.06em] uppercase">
                  {stat.label}
                </div>
              </div>
            ))}
          ...
          {[...Array(2)].map((_, i) => (
            <div key={i} className="flex items-center">
              {[
                ...
                <div
                  key={j}
                  className="flex items-center gap-2.5 px-7 border-r border-rule font-mono text-[12px] text-faint whitespace-nowrap"
                >
                  <span className="text-body">{item.n}</span>
                  {item.d && (
                    <span className={cn(item.up ? "text-green" : "text-amber")}>
                      {item.d}
                    </span>
                  )}
                  <span>·</span>
                  <span className={cn(item.cyan ? "text-teal" : "text-faint")}>
                    {item.l}
                  </span>
                </div>
              ))}
        ...
        <div className="max-w-[1200px] mx-auto">
          ...
          <div className="reveal">
            ...
            <h2 className="font-display text-[clamp(36px,5vw,64px)] font-extrabold tracking-[-0.035em] leading-[0.98] text-white mb-6">
              Payments are broken <br /> at{" "}
              <span className="font-serif font-light italic text-lead">
                the seams
              </span>
            </h2>
            <p className="font-serif font-light text-[18px] text-body leading-relaxed max-w-[520px]">
              Most businesses juggle four vendors to handle what should be one
              job. Every integration is a separate failure point — and a
              separate invoice.
            </p>
          </div>

          <motion.div
            className="grid grid-cols-1 md:grid-cols-2 gap-px bg-rule border border-rule rounded-2xl overflow-hidden mt-16"
            {...reveal}
          >
            <div className="bg-ink p-10 md:p-14">
              <span className="font-mono text-[11px] tracking-[0.08em] uppercase px-2.5 py-1 rounded-[5px] bg-red/10 text-red border border-red/20 inline-block mb-6">
                Today
              </span>
              <h3 className="font-display text-[26px] font-bold tracking-tight text-white mb-4 leading-tight">
                Four vendors.
                <br />
                Four failure points.
              </h3>
              <p className="font-serif font-light text-[15px] text-body leading-relaxed mb-8">
                Card payments, cross-border transfers, crypto acceptance, and
                cross-chain bridging each demand their own SDK, their own
                dashboard, and their own contract. The result is brittle
                infrastructure that slows every payment decision you make.
              </p>
              <ul className="space-y-3 font-serif font-light text-[15px] text-body list-none">
                {[
                  ...
                  <li key={li} className="flex gap-3 items-start">
                    <span className="text-red font-bold">✗</span> {li}
                  </li>
                ))}
              </ul>
            </div>
            <div className="bg-ink2 p-10 md:p-14">
              <span className="font-mono text-[11px] tracking-[0.08em] uppercase px-2.5 py-1 rounded-[5px] bg-teal/10 text-teal border border-teal/20 inline-block mb-6">
                With Useroutr
              </span>
              <h3 className="font-display text-[26px] font-bold tracking-tight text-white mb-4 leading-tight">
                One integration.
              ...
              <p className="font-serif font-light text-[15px] text-body leading-relaxed mb-8">
                Useroutr collapses the entire stack into a single SDK and API.
                Accept cards, bank transfers, and crypto from any chain. Pay out
                to any wallet, bank, or mobile money network in 174 countries.
                All settled through Stellar in seconds.
              </p>
              <ul className="space-y-3 font-serif font-light text-[15px] text-lead list-none">
                {[
                  ...
                  <li key={li} className="flex gap-3 items-start">
                    <span className="text-teal font-bold">✓</span> {li}
                  </li>
                ))}
              </ul>
            </div>
      ...
      <section
        id="how-it-works"
        className="py-32 md:py-48 px-6 md:px-10 bg-ink relative overflow-hidden"
      >
        ...
        <div className="max-w-[1200px] mx-auto relative z-10">
          <div className="max-w-2xl mb-24 reveal">
            ...
            <p className="font-serif font-light text-[19px] text-body leading-relaxed">
              Every payment routes through a Hash Time Locked Contract. Both
              sides complete — or both sides refund. There is no in-between.
            </p>
          </div>
            ...
            {[
              ...
              >
                ...
                <div className="bg-ink2/50 backdrop-blur-xl border border-rule rounded-2xl p-8 h-full transition-all duration-500 group-hover:border-rule2 group-hover:-translate-y-2 group-hover:shadow-2xl group-hover:shadow-blue/5">
                  ...
                  <h4 className="font-display text-[17px] font-bold text-white mb-4 leading-tight">
                    {step.t}
                  </h4>
                  ...
                  <div className="flex flex-wrap gap-2 mt-auto">
                    {step.tech.map((t, j) => (
                      <span
                        key={j}
                        className="font-mono text-[10px] text-faint bg-ink px-2 py-0.5 rounded-sm uppercase tracking-wider group-hover:text-body transition-colors"
                      >
                        {t}
                      </span>
                    ))}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-start">
            <motion.div {...reveal}>
              ...
              <h2 className="font-display text-[44px] md:text-[56px] font-bold text-white leading-[0.95] tracking-tight mb-8">
                ...
                <span className="text-lead italic font-light drop-shadow-[0_0_15px_rgba(255,255,255,0.2)]">
                  designed for payments
                </span>
              </h2>
              <p className="font-serif font-light text-[20px] text-body leading-relaxed mb-12">
                Most infrastructure bolts crypto onto fiat, or fiat onto crypto.
                Useroutr is built natively where they intersect — on a
                blockchain that has processed real-world fiat flows for a
                decade.
              </p>
              ...
              <div className="grid grid-cols-2 gap-4">
                {[
                  ...
                  <div
                    key={i}
                    className="bg-ink border border-rule rounded-xl p-6 transition-colors hover:border-rule2"
                  >
                    <div className="font-display text-[32px] font-bold text-white mb-1 tracking-tighter">
                      {s.n}
                    </div>
              ...
              {[
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="font-serif font-light text-[15px] text-body leading-relaxed">
                      {f.d}
                    </p>
                  </div>
                </motion.div>
        ...
        <div className="max-w-7xl mx-auto px-6 text-center">
          <motion.div {...reveal} className="mb-16">
            ...
            <h2 className="font-display text-[44px] md:text-[64px] font-bold text-white leading-none tracking-tight mb-6">
              ...
              <span className="text-lead italic font-light">
                Settle anywhere.
              </span>
            </h2>
            <p className="font-serif font-light text-[18px] text-body leading-relaxed max-w-2xl mx-auto">
              Every chain routes through Stellar as the settlement hub. The
              bridge provider is chosen automatically — Circle CCTP, Wormhole,
              or Layerswap.
            </p>
          </motion.div>
            ...
            {[
              ...
              >
                <div
                  ...
                  style={{ backgroundColor: chain.c }}
                />
                <span className="font-display text-[14px] font-semibold text-lead">
                  {chain.n}
                </span>
                <span className="font-mono text-[9px] text-faint bg-ink px-2 py-0.5 rounded-sm uppercase group-hover:text-body transition-colors">
                  {chain.b}
                </span>
              </div>
            ))}
          ...
          <p className="font-mono text-[12px] text-faint">
            Stellar is the hub. Every payment settles via{" "}
            <span className="text-teal/80">path payment network</span> before
      ...
      <section
        id="developers"
        className="py-32 md:py-48 px-6 md:px-10 bg-ink2 border-y border-rule"
      >
            ...
            <div className="grid grid-cols-1 gap-8">
              {[
                [
                  "01",
                  "Sandbox by default",
                  "Run the full cross-chain flow with testnet funds instantly.",
                ],
                [
                  "02",
                  "TypeScript-first SDK",
                  "First-class types throughout. Mirroring the API exactly.",
                ],
                [
                  "03",
                  "Retrying Webhooks",
                  "Exponential backoff and HMAC-SHA256 verification.",
                ],
              ].map(([num, title, text]) => (
                <div
                  key={num}
                  className="flex gap-6 pb-8 border-b border-rule last:border-0 last:pb-0"
                >
                  <span className="font-mono text-[12px] text-faint pt-1">
                    {num}
                  </span>
                  <div className="space-y-2">
                    ...
                    <p className="font-serif font-light text-[14px] text-body leading-relaxed">
                      {text}
                    </p>
                  </div>
                </div>
        ...
        <div className="max-w-[1200px] mx-auto relative z-10">
          <div className="text-center max-w-2xl mx-auto mb-20 reveal">
            ...
            <p className="font-serif font-light text-[18px] text-body leading-relaxed">
              Transaction fees only. No monthly minimums. No setup fees. No
              hidden currency conversion markup. Pay for what you process.
            </p>
          </div>
            ...
            {[
              ...
              },
              {
                ...
                f: [
                  ...
                  "Global payouts · 174 countries",
                  "Fiat on/off ramp (MoneyGram)",
              ...
              <motion.div
                ...
                className={cn(
                  ...
                  plan.primary
                    ? "border-blue/40 shadow-xl shadow-blue/10 scale-105 z-10"
              ...
              >
                ...
                <div className="font-mono text-[11px] text-faint uppercase tracking-wider mb-6">
                  {plan.t}
                </div>
                ...
                <div className="mb-8">
                  <div className="flex items-baseline gap-1">
                    {!plan.isContact && (
                      <span className="font-display text-[24px] font-bold text-body">
                        %
                      </span>
                    )}
                    <span
                  ...
                  <div className="font-serif font-light text-[13px] text-body mt-2">
                    {plan.s}
                  </div>
                </div>
                ...
                <ul className="space-y-4 mb-10 grow">
                  {plan.f.map((feat, j) => (
                    <li
                      key={j}
                      className="flex gap-3 items-start font-serif font-light text-[14px] text-lead"
                    >
                ...
                <a
                  href={
                    plan.isContact
                      ? "mailto:enterprise@useroutr.com"
                      : "#waitlist"
                  }
                  className={cn(
      ...
      >
        ...
        <motion.div className="relative z-1" {...reveal}>
          <div className="font-mono text-[12px] text-teal tracking-widest uppercase mb-6 flex items-center justify-center gap-2.5">
            <span className="opacity-40">—</span> Private access{" "}
          ...
          <p className="font-serif font-light text-[19px] text-body max-w-[500px] mx-auto mb-14">
            Join the builders shipping unified fiat and crypto payments across
            174 countries.
          </p>

          <form
            className="flex flex-col sm:flex-row gap-2.5 max-w-[440px] mx-auto mb-5"
          ...
          >
            ...
            />
            <button className="bg-blue hover:bg-blue2 text-white font-display text-[15px] font-bold px-7 py-3.5 rounded-[10px] transition-all whitespace-nowrap">
              Join the waitlist
            </button>
          </form>
      ...
      <footer className="bg-ink border-t border-rule px-6 md:px-10 py-16 md:py-20">
        <div className="max-w-[1200px] mx-auto grid grid-cols-1 md:grid-cols-[1.5fr_1fr_1fr_1fr] gap-12 md:gap-16 mb-16">
          <div>
            ...
            <p className="font-serif font-light text-[14px] text-body leading-relaxed max-w-[240px]">
              The payment layer for the future. Built non-custodial on the
              Stellar network.
            </p>
          </div>
          ...
          {[
            {
              label: "Network",
              links: ["Status", "Explorer", "Docs", "GitHub"],
            },
            { label: "Company", links: ["About", "Brand", "Contact", "Terms"] },
            {
              label: "Social",
              links: ["Twitter", "Discord", "LinkedIn", "YouTube"],
            },
          ].map((col, i) => (
            <div key={i}>
              <div className="font-mono text-[11px] tracking-widest uppercase text-faint mb-6">
                {col.label}
              </div>
              <ul className="list-none space-y-3">
                {col.links.map((link) => (
                  <li key={link}>
                    <a
                      href="#"
                      className="font-serif font-light text-[14px] text-[#4A5568] hover:text-lead transition-colors"
                    >
                      {link}
                    </a>
                  </li>
        ...
        <div className="max-w-[1200px] mx-auto pt-8 border-t border-rule flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-1.5 font-mono text-[12px] text-faint">
            Made with <span className="text-red">♥</span> for the global
            economy.
          </div>
        </div>
      </footer>
