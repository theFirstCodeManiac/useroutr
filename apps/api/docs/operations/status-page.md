...
```
                ...
                ┌────────────────┼─────────────────┐
                ▼                ▼                 ▼
       api.useroutr.com    app.useroutr.com   useroutr.com
          /readyz                /                 /
              │
              └──> Postgres + Redis + Stellar Horizon probes
                   (inline, < 3s budget)
...
`/readyz` fans out to Postgres (`SELECT 1`), Redis (`PING`), and Stellar
Horizon (HEAD `/`) with a 3-second cap on the Stellar call. Response body:
...
Better Stack itself has no env presence in this app — it just pings public
HTTPS endpoints. If we later want to push incidents from the API into
Better Stack (auto-create when a queue fills up), we'll add a
`BETTERSTACK_INGEST_KEY` then.
...
- [ ] Add 5 monitors per the table above
- [ ] Create the status page with 4 components
